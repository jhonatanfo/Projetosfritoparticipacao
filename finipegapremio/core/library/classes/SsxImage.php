...
class SsxImage
{
    ...
    public function resize($newX,$newY)
    {
     ...
     public function resizeRatioWidth($newX,$newY,$transparente=false)
    {
        ...
        $ratio_orig = $this->x/$this->y;

        if($this->y > $this->x){
            $newX = $newY * $ratio_orig;
        }else{
           $newY = $newX/$ratio_orig;
        }
        ...
        if($this->resource)
        {
            $newImg = imagecreatetruecolor($oldX,$oldY);
            $canvas = imagecolorallocate($newImg, 255, 255, 255);
            if($this->mime == 3){
                $canvas = imagecolorallocatealpha($newImg, 0, 0, 0, 127);
            }

            imagefill($newImg, 0, 0, $canvas);
            $x =0;
            $y = 0;
            ...
            if($transparente && $this->mime == 3){
               imagecolortransparent($newImg, imagecolorallocatealpha($newImg, 0, 0, 0, 127));
               imagealphablending($newImg, false );
               imagesavealpha( $newImg, true );
            }

            imagecopyresampled($newImg,$this->resource,$x,$y,0,0,$newX,$newY,$this->x,$this->y);
            $this->resource = $newImg;
            $this->x = $newX;
            $this->y = $newY;
    ...
    public function resizeRatioHeight($newX,$newY,$transparente=false)
    {
