...
class SsxModelSqlite{
	...
	public function __construct($file_db_path,$table=null,$primaryKeyColumn=null){

		if($file_db_path && is_file($file_db_path)){

			$this->sqlite = new SQLite3($file_db_path,SQLITE3_OPEN_READWRITE);

			$this->sqlite->createFunction('REGEXPSQLITE', 'REGEXPSQLITE');

			$this->sqlite->createFunction('REPLACENAMEAUDIO','REPLACENAMEAUDIO');	

			$this->sqlite->createFunction('REPLACETONOASCIISTRING', 'REPLACETONOASCIISTRING');
			
			$this->sqlite->createFunction('HASNUMBERINSTRING','HASNUMBERINSTRING');

			$this->sqlite->createFunction('GETDATEPAGAMENTOBYDATEANDOS','GETDATEPAGAMENTOBYDATEANDOS');

			$this->sqlite->createFunction('GETNUMBERSINSTRING','GETNUMBERSINSTRING');
			...
			die('Não foi possível conexão com o banco de dados!');
		}
	...
	public function update($id,Array $data,$rowAffected=null){
		...
		if(!$ret){
			...
			if($rowAffected != null){
				$rowAffected = $this->sqlite->changes();
				return $rowAffected;
			}else{
				return true;
			}
	...
	public function checkHasTable($table_name){
