...
class SsxModels extends SsxFilterDispatcher implements SsxModelsInterface
{
	...
	protected function simpleWhere($args,$teste=false)
	{
	 ...
	 * );
	 */
	public function filterData($args="", $one=false,$teste=false)
	{
		$where = self::simpleWhere($args);
		
		if($where){
			$where = "WHERE " . $where;
		}

		$fields_string = self::field_string();
