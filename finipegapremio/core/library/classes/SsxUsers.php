...
class SsxUsers extends SsxModels
{
	...
	public function auth($user, $pass, $cookie_verification=true,$teste=false)
	{
		global $Ssx;
	...
	public static function recoverPassOld($email, $url_callback="")
	{
	...
	public static function recoverPass($email,$url_callback=""){
		...
		$auth = SsxUsers::getByEmailStatic($email);
		if(!$auth)
			return false;
		...
		$dados_template['callback'] = $callback;
		$dados_template['authUser'] = $auth['user'];

		$SsxMail = new SsxMail();
		
		$body = "<h2>Recuperação de senha:</h2>
				 Você solicitou alteração de senha dá area administrativa<br/><br/>
				 Seu login é: <b>".$dados_template['authUser']."</b><br/>	
				 Acesse ".$dados_template["callback"]." para alterar sua senha <br/><br/>
				 Equipe Fri.to";		
		$mail = new PHPMailer();
		...
		$mail->SetFrom('no-reply@fri.to','Área administrativa fri.to');// Mandado Por
		$mail->Subject    = 'Recuperação de Senha - Área administrativa';
		//$mail->AltBody    = ""; // optional, comment out and test
		$mail->MsgHTML($body); // mandar o corpo da mensagem
		$mail->AddAddress($email,$auth['name']); //Setar email que vai ser enviado a mensagem 
		if(!$mail->Send()){
			return false;
		}else{
			return true;
	...
	public static function getByEmailStatic($email){
		$class = get_called_class();
		$self  = new $class();
		$sql = sprintf("SELECT id,user,name,email FROM %s WHERE deleted = 0 AND email ='%s'", $self->table_name,$email);
		return $self->link->getone($sql);
	}
